...
def main():
    ...
    while num_complete < args.num_samples:
        ...
        mean, log_variance, pred_xstart, sample = sample_fn(
            model,
            (args.batch_size, args.in_channels, args.image_size),
            #noise=noise,
        ...
        gather_and_save_samples(mean, curr_batch, "mean")
        gather_and_save_samples(log_variance, curr_batch, "log_variance")
        gather_and_save_samples(pred_xstart, curr_batch, "pred_xstart")
        gather_and_save_samples(sample, curr_batch, "sample")
        if args.class_cond:
...
def gather_and_save_samples(sample, curr_batch, sample_name):
    ...
    if dist.get_rank() == 0:
        ...
        out_path = os.path.join(
            ...
            f"batch{curr_batch:03d}-{sample_name}s_history_{shape_str}.npz"
        )
